...
//#include "rajectoryDBAccessList.h"

//#include "opencv2/core/core.hpp"
#include "opencv2/imgproc/imgproc.hpp"
#include "opencv2/highgui/highgui.hpp"

#include <C:/Boost/vc14/include/boost-1_61/boost/graph/connected_components.hpp>
...
float FeatureTrajectory::getMainDirc()
{
	int nSize = positions->size();
	Mat A( nSize, 2, CV_32FC1 );
	double x = 0;
	double y = 0;
	for ( int i = 0; i < nSize; i++ )
	{
		A.at<float>( i, 0 ) = ( *positions )[i].x;
		A.at<float>( i, 1 ) = ( *positions )[i].y;
		x += A.at<float>( i, 0 );
		y += A.at<float>( i, 1 );
	}
	x /= nSize;
	...
	Mat mat1 = Mat::ones( nSize, 2, CV_32FC1 );
	Mat mean1 = Mat::zeros( 2, 2, CV_32FC1 );
	mean1.at<float>( 0 ) = x;
	mean1.at<float>( 3 ) = y;
	...
	float dAngle = 180.0/CV_PI * atan2( VT.at<float>( 0 ), VT.at<float>( 1 ) );
...
bool FeatureTrajectory::getPts( cv::Point2f& pt, unsigned int nFrameID )
{
	nFrameID -= firstInstant;
	if ( nFrameID >= positions->size() )
		return false;

	pt =  ( *positions )[nFrameID];
	return true;
}
...
void FeatureTrajectory::GetLastPoint( double &x, double &y )
{
	if ( positions->size()<1 ) return;

	Point2f p = ( *positions )[positions->size()-1];
	x = p.x;
	y = p.y;
}
// protected

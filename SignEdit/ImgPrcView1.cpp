// 系统View.cpp : implementation of the CImgPrcView class
//
#include "stdafx.h"
#include "ImgPrc.h"
#include "ImgPrcDoc.h"
...
/*void FormCreate(  )
{
	...
	if ( JoyMsg == JOYERR_NOCANDO )
		ShowMessage( "不能捕获游戏杆!" );
	else if ( JoyMsg == JOYERR_UNPLUGGED )
			ShowMessage( "游戏杆未与系统连接!" );
	else if ( JoyMsg == MMSYSERR_NODRIVER )
...
{
	void OnJoyDown( Tmessage &Message )
	{
...
string g_strMaskFile = "F:\\mask.png";
...
CImgPrcView::CImgPrcView()
{
	...
	string g_strMaskFile = "F:\\mask.png";
	g_maskfixed = imread( g_strMaskFile );
	cvtColor( g_maskfixed, g_maskfixed, CV_RGB2GRAY );
...
void CImgPrcView::OnFileOpen()
{
	...
	static char szFilter[] = "|PNG文件(*.png)|*.png|MP4文件(*.mp4)|*.mp4|BMP文件(*.bmp)|*.bmp||";
	...
	CString strExt = dlg.GetFileExt();
	{
		...
		while ( bWorking )
		{
			...
			if ( !strPath.Compare( m_strFileName ) )
				g_nFileID = g_strFileNames.GetCount();
	...
	{	
		...
		//warpAffine( frame, frame, M, frame.size() );//仿射变换  
...
void CImgPrcView::OnFileOpen2() 
{
	...
	FreeInterResultImage();
	AddInterResultImage(m_pBitmap2,m_nHeight2,m_nWidth2,m_flag2,"第二幅图（右）");
...
LRESULT CImgPrcView::OnPolygon(WPARAM wParam, LPARAM lParam)
{
	...
	for (size_t i = 0; i < contours.size(); ++i)
	{
		...
		Mat pointsf;
		Mat(contours[i]).convertTo(pointsf, CV_32F);
		RotatedRect ellipse = fitEllipse(pointsf);	//椭圆拟合
...
LRESULT CImgPrcView::OnConerDetect(WPARAM wParam, LPARAM lParam)
{
	...
	{ 
		...
		cv::goodFeaturesToTrack(img, corners,
			m_pConer->m_nMaxNum,     //角点最大数目  
			m_pConer->m_dThreshold,  // 质量等级，这里是0.01*max（min（e1，e2）），e1，e2是harris矩阵的特征值  
			m_pConer->m_nMinDistance); // 两个角点之间的距离容忍度  

		cvtColor( img, img, CV_GRAY2RGB );
		proc::Harris().drawOnImage(img, corners, cv::Scalar( 0, 0, 255 ));//标记角点  
		break;
	}
	case 1:	//harris角点
	{
		...
		//标记角点
		cvtColor( img, img, CV_GRAY2RGB );
		harris.drawOnImage( img, pts, cv::Scalar( 0, 0, 255 ) );//标记角点  
		break;
	}
	case 2:	//快速角点检测  
	{
...
LRESULT CImgPrcView::OnhdlSmooth(WPARAM wParam, LPARAM lParam)
{
	...
	//CString strTmp;
	//strTmp.Format("形态学， 结构元半径: %d", kernel_type);
...
void CImgPrcView::OnCreateHistDialog()
{
	...
	if (!m_pBitmap)
	{
		MessageBox("请加载图片.", MB_OK);
		return;
	}
	if (m_flag == 24)
	{
		MessageBox("仅处理灰度图像.", MB_OK);
		return;
	}
...
void CImgPrcView::RemoveCamShake2(Mat &referImg, Mat &img, double&x, double&y)
{
	...
	if ( bFirst )
	{
		...
		goodFeaturesToTrack( refer, prevPts, 32,    // 角点最大数目
...
void CImgPrcView::OnSignRecognize()
{
	...
	//for ( int iii = 4; iii < 350; iii++ )
	{
		...
		/*if ( i < 42 )
		{
			double dRatio = i / 42.0 ;
			rct1.x = dRatio*( 1015 - 1015 ) + 1015;
			rct1.y = dRatio*( 342 - 388 ) + 388;
			rct1.width = dRatio*( 160 - 120 ) + 120;
			rct1.height = dRatio*( 160 - 120 ) + 120;

		}
		else if ( i < 100 )
		{
			double dRatio = ( i - 42 ) / ( 100.0 - 42.0 );
			rct1.x = dRatio*( 1052 - 1015 ) + 1015;
			rct1.y = dRatio*( 196 - 342 ) + 342;
			rct1.width = dRatio*( 250 - 160 ) + 160;
			rct1.height = dRatio*( 250 - 160 ) + 160;
		}
		else if ( i < 120 )
		{
			double dRatio = ( i - 100 ) / ( 120.0 - 100.0 );
			rct1.x = dRatio*( 1090 - 1052 ) + 1052;
			rct1.y = dRatio*( 80 - 196 ) + 196;
			rct1.width = dRatio*( 300 - 250 ) + 250;
			rct1.height = dRatio*( 300 - 250 ) + 250;
		}
		else if ( i < 127 )
		{
			double dRatio = ( i - 120 ) / ( 127.0 - 120.0 );
			rct1.x = dRatio*( 1108 - 1090 ) + 1090;
			rct1.y = dRatio*( 20 - 80 ) + 80;
			rct1.width = dRatio*( 360 - 300 ) + 300;
			rct1.height = dRatio*( 360 - 300 ) + 300;
		}*/
...
void UpdateLocalPt( Mat& matS, Mat& matD, double dx, double dy, int nds, int ndy, double dRatio )
{
	for ( int k = 0; k < 3; k++ )
	{
		...
		matD.data[( ndy*matD.cols + nds ) * 3 + k] = temp * dRatio + matD.data[( ndy*matD.cols + nds ) * 3 + k] * ( 1 - dRatio );
	}
}
...
UINT CImgPrcView::SignMosaic( LPVOID lp )
{
	...
	while ( pos0 )
	{
		...
		resize( imgSign0, imgSign, Size( ( nR - nL ) * 2, ( nB - nT ) * 2 ), 0, 0, INTER_AREA );
		GaussianBlur( imgSign, imgSign, Size( 3, 3 ), 0.6 );
		...
		double dRatio = pDlg->m_dRatio;// 3.6;
		...
		for ( int i = 0; i < 4; i++ )
		{
			pt[i].x = ( pt[i].x - dX ) * dRatio + dX;
			pt[i].y = ( pt[i].y - dY ) * dRatio + dY;
		}
		...
		f1.at<float>( 0, 0 ) = pt[1].x * nWidthSet  / (double) nW;
		f1.at<float>( 0, 1 ) = pt[1].y * nHeightSet / (double) nH;
		f1.at<float>( 1, 0 ) = pt[2].x * nWidthSet  / (double) nW;
		f1.at<float>( 1, 1 ) = pt[2].y * nHeightSet / (double) nH;
		f1.at<float>( 2, 0 ) = pt[3].x * nWidthSet  / (double) nW;
		f1.at<float>( 2, 1 ) = pt[3].y * nHeightSet / (double) nH;
		f1.at<float>( 3, 0 ) = pt[0].x * nWidthSet  / (double) nW;
		f1.at<float>( 3, 1 ) = pt[0].y * nHeightSet / (double) nH;
		...
		for ( int i = 0; i < imgs.rows; i++ )
		{
			...
			if ( x0 >= imgs.cols ) continue;

			double u0[9], v0[9];
			double u1[9], v1[9];
			CalProjectPt( pM, x0, i, u0, v0 );
			CalProjectPt( pM, x1, i, u1, v1 );

			double dRatio = 1;

			if ( u0[3] < 0 )
				UpdateLocalPt( imgSign, imgs, u0[4], v0[4], x0 - 1, i, u0[4] / ( u0[4] - u0[3] ) );

			if ( u1[5] >= imgSign.cols )
				UpdateLocalPt( imgSign, imgs, u1[4], v1[4], x1, i, ( imgSign.cols - u1[4] ) / ( u1[5] - u1[4] ) );

			if ( v0[1] < 0 )
				UpdateLocalPt( imgSign, imgs, u0[4], v0[4], x0 - 1, i, v0[4] / ( v0[4] - v0[1] ) );


			for ( int j = x0; j < x1; j++ )
			{
				double y0 = ProjectY( pM, j, i - 1 );
				double y1 = ProjectY( pM, j, i + 1 );
				...
				if ( y0 < 0 )
				{
					CalProjectPt( pM, j, i, u0, v0 );
					UpdateLocalPt( imgSign, imgs, x, y, j, i, v0[4] / ( v0[4] - v0[1] ) );
				}
				else if ( y1 >= imgSign.rows )
				{
					CalProjectPt( pM, j, i, u0, v0 );
					UpdateLocalPt( imgSign, imgs, x, y, j, i, ( imgSign.rows - v0[4] ) / ( v0[4] - v0[1] ) );
				}
				else
					UpdateLocalPt( imgSign, imgs, x, y, j, i, 1 );
			}
		}
